<template>
  <div
    class="relative w-full min-h-screen flex justify-center items-center bg-[url('/img/bglogin2.webp')] bg-cover bg-center"
  >
    <!-- overlay -->
    ...
    <!-- CARD -->
    <div
      class="relative z-20 w-full max-w-[380px] sm:max-w-[520px] lg:max-w-[720px] bg-white/45 backdrop-blur-md rounded-[32px] sm:rounded-[48px] lg:rounded-[60px] shadow-2xl p-5 sm:p-6 lg:p-8"
    >
      <div class="flex flex-col items-center gap-4">
        <h1
          class="text-2xl sm:text-3xl lg:text-4xl font-bold text-white text-shadow"
        >
          เลือกแท็กภาพยนตร์ที่คุณชอบ!
        </h1>

        <h2 class="text-sm sm:text-base lg:text-lg text-white text-center">
          เลือกแท็กที่คุณชื่นชอบ 3 แท็ก
          แล้วให้เราช่วยแนะนำภาพยนตร์ที่เหมาะกับคุณ
        </h2>

        <!-- TAG GRID -->
        <div
          class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 w-full max-w-[300px] sm:max-w-[520px] mt-4"
        >
          <button
            v-for="tag in tags"
            :key="tag.id"
        ...
        <!-- REGISTER BUTTON -->
        <button
          class="mt-6 w-[220px] h-[46px] bg-[#A0E13E] text-black rounded-full font-bold shadow-xl hover:bg-[#80b432] disabled:opacity-40 disabled:cursor-not-allowed"
